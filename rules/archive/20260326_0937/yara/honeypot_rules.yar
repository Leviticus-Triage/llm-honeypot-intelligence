...
rule Honeypot_SystemRecon {
    ...
    strings:
        ...
        $s6 = "uname -s -m" ascii nocase
        $s7 = "ifconfig" ascii nocase
        $s8 = "ip route show 2>/dev/null" ascii nocase
        $s9 = "uname -p 2>/dev/null" ascii nocase
        $s10 = "lscpu 2>/dev/null" ascii nocase
        $s11 = "ps aux" ascii nocase
    condition:
        any of them
}
