...
`192.168.2.116`, T-Pot VM 400 auf Proxmox (`192.168.2.22`).
...
| Phase 6 Offline-ML Runner (IsoForest + Classifier) | ✅ Live auf `.116`: `ml-runner` Export/Train/Infer erfolgreich, `honeypot-cve-sessions` mit `ml_anomaly_score` + `ml_classifier_tags` angereichert | `proxy/src/ml_runner.py`, `proxy/run_ml_runner.py`, `proxy/docker-compose.yml` |
...
| Phase 2.5 Erste Kalibrierung live angewendet | ✅ 25 Sidecars auf `.116` gesweept (12 labelled: 5 tp / 7 fp). Befund: `LLM_CONF_THRESHOLD=0.70` sitzt auf Precision/Recall=1.0-Plateau bis 0.80, Cliff bei 0.825 → unverändert gelassen. `DEDUPE_THRESHOLD=0.985` fing nur 2 von 6 Dupes → auf **0.93** gesenkt (retained_recall=1.0, 0 legit drops). Container recreate `failures=0`. | `proxy/docker-compose.yml`, `validated-rules/.state/calibration.{json,md}` |
...
| Phase 2.5 Label-Path Fix + Seed-Labeling v2 | ✅ Kalibrator liest Labels jetzt korrekt vom **sidecar-adjacent Rule-Path** (statt nur `source_path`), dadurch werden Labels unter `validated-rules/**` wirklich ausgewertet. Live auf `.116`: 13 explizite Labels (`tp=5`, `fp=8`, 12 unlabeled), Empfehlung stabil bei `LLM_CONF_THRESHOLD=0.65` und `DEDUPE_THRESHOLD=0.89` unter `MAX_FP_RATE=0.05`. | `proxy/calibrate_thresholds.py`, `proxy/label_rules.py`, `validated-rules/.state/calibration.{json,md}` |
