...
```
 ...
 ┌─────▼──────── Proxmox station-2 ──────────▼─────┐
 │                                                 │
 │   VM 400  tpot-honeypot (192.168.2.22)          │
 │   ┌─────────────────────────────────────────┐   │
 │   │ T-Pot Docker (beelzebub, galah, ddospot,│   │
 │   │ suricata, kibana, elasticsearch,        │   │
 │   │ logstash, ...)                          │   │
 │   │   └── ollama_upstream ──▶ 192.168.2.116 │   │
 │   └─────────────────────────────────────────┘   │
 │                                                 │
 │   VM ai-workstation (192.168.2.116)             │
 │   ┌─────────────────────────────────────────┐   │
...
ml-runner:
  ...
  network_mode: host
  environment:
    - ES_URL=https://192.168.2.22:64297/es
...
1. Modell in Ollama gepullt (`ollama list` auf `.116`).
...
1. `engine.py` → `/home/danii/ollama-proxy/src/c2_detection/`.
...
4. **Human-in-the-loop**: Soll es ein `rules-review`-UI geben, in dem
   ...
   Streamlit-Dashboard auf `.116:8502`.
